...
def PRM_planning(sx, sy, gx, gy, ox, oy, rr,N_SAMPLE):

    obkdtree = KDTree(np.vstack((ox, oy)).T)

    sample_x, sample_y = sample_points(sx, sy, gx, gy, rr, ox, oy, obkdtree,N_SAMPLE)
    if show_animation:
...
def sample_points(sx, sy, gx, gy, rr, ox, oy, obkdtree,N_SAMPLE):
    maxx = max(ox)
    maxy = max(oy)
    minx = min(ox)
    miny = min(oy)
...
def NAV(sx, sy, gx, gy, ox, oy, robot_size,show_animation,N_SAMPLE):
    if show_animation:
        plt.plot(ox, oy, ".k")
        plt.plot(sx, sy, "^r")
        plt.plot(gx, gy, "^c")
